...
async def initiate_return(order_id: str, product_id: str, customer_id: str, reason: str) -> Optional[OrderItem]:
    logger.info(
        "initiate_return_called",
        extra={"event": "initiate_return_called", "order_id": order_id, "customer_id": customer_id},
    )
    order: Optional[Order] = await get_order_by_id(order_id)
